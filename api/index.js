...
// ============================================================================
// AD STUDIO PRO — ADDITIVE FIRST-PARTY ADVERTISING ENGINE
// Existing campaign, sponsored-task, economy, and game routes remain intact.
// ============================================================================
function aspNumber(value, fallback = 0) {
    const n = Number(value);
    return Number.isFinite(n) && n >= 0 ? n : fallback;
}
function aspValidUrl(value) {
    try {
...
function aspSummary(c) {
    return {...c, impressions:Number(c.impressions||c.views||0), adClicks:Number(c.adClicks||0), adSpend:Number(c.adSpend||0), escrowReserved:Number(c.escrowReserved||0)};
}
app.get('/api/ad-studio/dashboard/:userId', async (req,res)=>{
    try {
        ...
        const campaigns=Object.values(all).filter(c=>String(c.userId)===userId).map(aspSummary).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));
        res.json({user,campaigns});
    } catch(e) { console.error('[Ad Studio dashboard]',e.message); res.status(500).json({error:'Unable to load Ad Studio'}); }
});
app.post('/api/ad-studio/campaigns', async (req,res)=>{
    try {
        const b=req.body||{}, userId=String(b.userId||''), mode=['advertisement','task','hybrid'].includes(b.mode)?b.mode:'advertisement';
        const name=String(b.name||'').trim().slice(0,80), link=String(b.link||'').trim(), imageUrl=String(b.imageUrl||'').trim(), videoUrl=String(b.videoUrl||'').trim();
        const wantsAd=mode!=='task', wantsTask=mode!=='advertisement';
        if(!userId||!name||!aspValidUrl(link))return res.status(400).json({error:'Campaign name and a valid destination URL are required'});
        if(wantsAd&&!imageUrl&&!videoUrl)return res.status(400).json({error:'Advertisement requires an image or video URL'});
        if(imageUrl&&!aspValidUrl(imageUrl))return res.status(400).json({error:'Image URL must be HTTP or HTTPS'});
        if(videoUrl&&!aspValidUrl(videoUrl))return res.status(400).json({error:'Video URL must be HTTP or HTTPS'});
        const user=await dbGet(`users/${userId}`);
        if(!user)return res.status(404).json({error:'User not found'});
        if(user.isBanned)return res.status(403).json({error:'Account is banned'});
        const maxUsers=wantsTask?Math.floor(aspNumber(b.maxUsers)):0, reward=wantsTask?Math.floor(aspNumber(b.reward)):0, impressionBudget=wantsAd?Math.floor(aspNumber(b.impressionBudget)):0;
        if(wantsTask&&(maxUsers<1||reward<1))return res.status(400).json({error:'Task target and reward must be greater than zero'});
        if(wantsAd&&impressionBudget<1)return res.status(400).json({error:'Impression budget must be greater than zero'});
        const config=await dbGet('config')||{}, cpi=Math.max(1,Math.floor(aspNumber(config.adStudioCostPerImpression,1)));
        const adReserve=impressionBudget*cpi, taskReserve=maxUsers*reward, reserve=adReserve+taskReserve;
        if(Number(user.points||0)<reserve)return res.status(400).json({error:`Not enough Gems. Required ${reserve}, available ${Number(user.points||0)}`});
        const id=`ad_${Date.now()}_${Math.random().toString(36).slice(2,8)}`, now=Date.now();
        const campaign={id,userId,name,link,desc:String(b.desc||'').trim().slice(0,500),icon:String(b.icon||'fa-rectangle-ad').slice(0,40),type:wantsTask?(b.taskType==='manual'?'manual':'auto'):'advertisement',isAdvertisement:wantsAd,sponsoredTask:wantsTask,adFormat:videoUrl?'video':'image',imageUrl,videoUrl,headline:String(b.headline||'').trim().slice(0,100),cta:String(b.cta||'Visit Now').trim().slice(0,32),taskType:String(b.taskType||'').slice(0,20),maxUsers,reward,impressionBudget,dailyBudget:Math.floor(aspNumber(b.dailyBudget)),impressions:0,views:0,adClicks:0,adSpend:0,claims:0,paused:false,autoRenew:false,queue:[],costPerImpression:cpi,escrowReserved:reserve,adEscrowRemaining:adReserve,taskEscrowRemaining:taskReserve,createdAt:now,updatedAt:now};
        await dbSet(`campaigns/${id}`,campaign);
        await dbUpdate(`users/${userId}`,{points:Number(user.points||0)-reserve,stuckBalance:Number(user.stuckBalance||0)+reserve,logs:logAction(user,`Created Ad Studio PRO campaign ${id} (Locked ${reserve} Gems)`)});
        const all=await dbGet('campaigns')||{};
        const campaigns=Object.values(all).filter(c=>String(c.userId)===userId).map(aspSummary).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));
        res.json({success:true,campaignId:id,user:await dbGet(`users/${userId}`),campaigns});
    } catch(e) { console.error('[Ad Studio create]',e.message); res.status(500).json({error:'Unable to create campaign'}); }
});
app.post('/api/ad-studio/campaign/action', async (req,res)=>{
    try {
        const userId=String(req.body.userId||''), id=String(req.body.campaignId||''), action=String(req.body.action||''), c=await dbGet(`campaigns/${id}`);
        if(!c||String(c.userId)!==userId)return res.status(403).json({error:'Unauthorized campaign'});
        if(action==='toggle')await dbUpdate(`campaigns/${id}`,{paused:!Boolean(c.paused),updatedAt:Date.now()});
        else if(action==='liquidate'){
            const u=await dbGet(`users/${userId}`);if(!u)return res.status(404).json({error:'User not found'});
            const refund=Number(c.adEscrowRemaining||0)+Math.max(0,(Number(c.maxUsers||0)-Number(c.claims||0))*Number(c.reward||0));
            await dbUpdate(`users/${userId}`,{points:Number(u.points||0)+refund,stuckBalance:Math.max(0,Number(u.stuckBalance||0)-refund),logs:logAction(u,`Liquidated Ad Studio PRO campaign ${id} (Refunded ${refund} Gems)`)});
            await dbRemove(`campaigns/${id}`);
            return res.json({success:true,refunded:refund,user:await dbGet(`users/${userId}`),campaigns:[]});
        } else return res.status(400).json({error:'Unknown campaign action'});
        const all=await dbGet('campaigns')||{}, campaigns=Object.values(all).filter(x=>String(x.userId)===userId).map(aspSummary).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));
        res.json({success:true,user:await dbGet(`users/${userId}`),campaigns});
    } catch(e) { console.error('[Ad Studio action]',e.message); res.status(500).json({error:'Campaign action failed'}); }
});
app.get('/api/ad-studio/inventory', async (req,res)=>{
    try {
        const all=await dbGet('campaigns')||{};
        res.json(Object.values(all).filter(c=>c.isAdvertisement&&!c.paused&&Number(c.impressionBudget||0)>Number(c.impressions||0)).sort((a,b)=>Number(b.priority||0)-Number(a.priority||0)).map(aspSummary));
    } catch(e) { res.status(500).json({error:'Unable to load ad inventory'}); }
});
app.post('/api/ad-studio/impression', async (req,res)=>{
    try {
        const id=String(req.body.campaignId||''), c=await dbGet(`campaigns/${id}`);
        if(!c||!c.isAdvertisement||c.paused)return res.status(404).json({error:'Advertisement unavailable'});
        const budget=Number(c.impressionBudget||0), impressions=Number(c.impressions||0), cost=Math.max(1,Number(c.costPerImpression||1)), remaining=Number(c.adEscrowRemaining||0);
        if(impressions>=budget||remaining<cost)return res.status(409).json({error:'Advertisement budget exhausted'});
        const next=impressions+1;
        await dbUpdate(`campaigns/${id}`,{impressions:next,views:next,adSpend:Number(c.adSpend||0)+cost,adEscrowRemaining:remaining-cost,escrowReserved:Math.max(0,Number(c.escrowReserved||0)-cost),updatedAt:Date.now()});
        const sponsor=await dbGet(`users/${c.userId}`);
        if(sponsor)await dbUpdate(`users/${c.userId}`,{stuckBalance:Math.max(0,Number(sponsor.stuckBalance||0)-cost)});
        res.json({success:true,impressions:next});
    } catch(e) { res.status(500).json({error:'Impression could not be recorded'}); }
});
app.post('/api/ad-studio/click', async (req,res)=>{
    try {
        const c=await dbGet(`campaigns/${String(req.body.campaignId||'')}`);
        if(!c||!c.isAdvertisement)return res.status(404).json({error:'Advertisement unavailable'});
        await dbUpdate(`campaigns/${c.id}`,{adClicks:Number(c.adClicks||0)+1,updatedAt:Date.now()});
        res.json({success:true});
    } catch(e) { res.status(500).json({error:'Click could not be recorded'}); }
});
...
// ============================================================================
// AD STUDIO PRO DEEP BACKEND V3
// Additive production services. Existing MYFA BIRR endpoints are preserved.
// ============================================================================

const AD_STUDIO_PRO_V3 = {
    version: '3.0.0',
    lockMap: new Map(),
    maximums: {
        name: 80,
        ...
        users: 1000000,
        reward: 100000000,
        impressionBudget: 100000000,
        dailyBudget: 100000000
    }
};

function aspV3String(value, maximum) {
    return String(value ?? '').trim().slice(0, maximum);
}

function aspV3Number(value, fallback = 0) {
    const numberValue = Number(value);
    return Number.isFinite(numberValue) ? numberValue : fallback;
}

function aspV3Int(value, fallback = 0) {
    return Math.floor(Math.max(0, aspV3Number(value, fallback)));
}

function aspV3Https(value) {
    try {
...
function aspV3Mode(mode) {
    return ['advertisement', 'task', 'hybrid'].includes(String(mode));
}

function aspV3Format(campaign) {
...
function aspV3Status(campaign) {
    if (!campaign) return 'missing';
    if (campaign.paused) return 'paused';

    if (
        campaign.isAdvertisement &&
        aspV3Int(campaign.impressionBudget) > 0 &&
        aspV3Int(campaign.impressions || campaign.views) >=
            aspV3Int(campaign.impressionBudget)
    ) {
        return 'completed';
    }
    ...
    if (
        ...
        aspV3Int(campaign.maxUsers) > 0 &&
        aspV3Int(campaign.claims) >= aspV3Int(campaign.maxUsers)
    ) {
        return 'completed';
    }
...
function aspV3Normalize(campaign) {
    return {
        ...campaign,
        impressions: aspV3Int(campaign?.impressions || campaign?.views),
        views: aspV3Int(campaign?.impressions || campaign?.views),
        adClicks: aspV3Int(campaign?.adClicks),
        adSpend: aspV3Int(campaign?.adSpend),
        escrowReserved: aspV3Int(campaign?.escrowReserved),
        adEscrowRemaining: aspV3Int(campaign?.adEscrowRemaining),
        taskEscrowRemaining: aspV3Int(campaign?.taskEscrowRemaining),
        claims: aspV3Int(campaign?.claims),
        maxUsers: aspV3Int(campaign?.maxUsers),
        reward: aspV3Int(campaign?.reward),
        impressionBudget: aspV3Int(campaign?.impressionBudget),
        dailyBudget: aspV3Int(campaign?.dailyBudget),
        status: aspV3Status(campaign),
        format: aspV3Format(campaign)
    };
}

function aspV3Lock(key, callback) {
    const previous = AD_STUDIO_PRO_V3.lockMap.get(key) || Promise.resolve();
    let release;
    ...
    AD_STUDIO_PRO_V3.lockMap.set(key, current);
    ...
    return previous.then(async () => {
        try {
            ...
            if (AD_STUDIO_PRO_V3.lockMap.get(key) === current) {
                AD_STUDIO_PRO_V3.lockMap.delete(key);
            }
        }
    });
}

async function aspV3User(userId) {
    return dbGet(`users/${String(userId || '')}`);
}

async function aspV3Campaign(campaignId) {
    return dbGet(`campaigns/${String(campaignId || '')}`);
}

async function aspV3Owned(userId) {
    const all = await dbGet('campaigns') || {};

    return Object.values(all)
        .filter(campaign => String(campaign.userId) === String(userId))
        .map(aspV3Normalize)
        .sort(
            (a, b) =>
                aspV3Int(b.updatedAt || b.createdAt) -
                aspV3Int(a.updatedAt || a.createdAt)
        );
}

function aspV3Errors(body) {
    const errors = [];
    const mode = String(body.mode || 'advertisement');

    if (!aspV3Mode(mode)) {
        errors.push('Invalid campaign mode.');
        return errors;
    }

    const name = aspV3String(body.name, AD_STUDIO_PRO_V3.maximums.name);
    const link = aspV3String(body.link, AD_STUDIO_PRO_V3.maximums.url);

    if (!name) errors.push('Campaign name is required.');
    if (!aspV3Https(link)) errors.push('Destination URL must use HTTPS.');

    if (mode !== 'task') {
        const imageUrl = aspV3String(body.imageUrl, AD_STUDIO_PRO_V3.maximums.url);
        const videoUrl = aspV3String(body.videoUrl, AD_STUDIO_PRO_V3.maximums.url);

        if (!imageUrl && !videoUrl) {
            errors.push('An image or video URL is required.');
        }

        if (imageUrl && !aspV3Https(imageUrl)) {
            errors.push('Image URL must use HTTPS.');
        }

        if (videoUrl && !aspV3Https(videoUrl)) {
            errors.push('Video URL must use HTTPS.');
        }

        if (aspV3Int(body.impressionBudget) < 1) {
            errors.push('Impression budget must be greater than zero.');
        }

        if (
            aspV3Int(body.impressionBudget) >
            AD_STUDIO_PRO_V3.maximums.impressionBudget
        ) {
            errors.push('Impression budget exceeds the maximum.');
        }
    }

    if (mode !== 'advertisement') {
        if (aspV3Int(body.maxUsers) < 1) {
            errors.push('Task target users must be greater than zero.');
        }

        if (aspV3Int(body.reward) < 1) {
            errors.push('Task reward must be greater than zero.');
        }

        if (
            aspV3Int(body.maxUsers) >
            AD_STUDIO_PRO_V3.maximums.users
        ) {
            errors.push('Task target users exceed the maximum.');
        }

        if (
            aspV3Int(body.reward) >
            AD_STUDIO_PRO_V3.maximums.reward
        ) {
            errors.push('Task reward exceeds the maximum.');
        }

        if (!['auto', 'manual'].includes(String(body.taskType || 'auto'))) {
            errors.push('Invalid task verification type.');
        }
    }

    if (
        aspV3Int(body.dailyBudget) >
        AD_STUDIO_PRO_V3.maximums.dailyBudget
    ) {
        errors.push('Daily budget exceeds the maximum.');
    }

    return [...new Set(errors)];
}

async function aspV3Reservation(body) {
    const config = await dbGet('config') || {};

    const costPerImpression = Math.max(
        1,
        aspV3Int(
            config.adStudioCostPerImpression,
            1
        )
    );
    ...
    const advertisement =
        mode !== 'task'
            ? aspV3Int(body.impressionBudget) * costPerImpression
            : 0;

    const task =
        mode !== 'advertisement'
            ? aspV3Int(body.maxUsers) * aspV3Int(body.reward)
            : 0;
    ...
    return {
        ...
        advertisement,
        task,
        total: advertisement + task
    };
}

function aspV3Audit(user, message, data = {}) {
    const base = typeof logAction === 'function'
...
app.get('/api/ad-studio/v3/status', async (req, res) => {
    res.json({
        service: 'ad-studio-pro',
        version: AD_STUDIO_PRO_V3.version,
        status: 'online',
        timestamp: Date.now()
    });
});

app.get('/api/ad-studio/v3/dashboard/:userId', async (req, res) => {
    try {
        const userId = String(req.params.userId || '');
        const user = await aspV3User(userId);
        ...
        const metrics = campaigns.reduce(
            (result, campaign) => {
                ...
                if (campaign.status === 'active') result.active += 1;
                if (campaign.status === 'paused') result.paused += 1;
                if (campaign.status === 'completed') result.completed += 1;
        ...
        console.error('[Ad Studio v3 dashboard]', error.message);
        res.status(500).json({ error: 'Unable to load advertiser dashboard' });
    }
});

app.get('/api/ad-studio/v3/analytics/:userId', async (req, res) => {
    try {
        const campaigns = await aspV3Owned(String(req.params.userId || ''));

        const byFormat = campaigns.reduce((result, campaign) => {
            const key = campaign.format;
            ...
            result[key].campaigns += 1;
            result[key].impressions += campaign.impressions;
            result[key].clicks += campaign.adClicks;
            result[key].spend += campaign.adSpend;

            return result;
        }, {});

        const leaderboard = [...campaigns]
            .sort((a, b) => b.adClicks - a.adClicks)
            .slice(0, 25)
            .map((campaign, index) => ({
                rank: index + 1,
                id: campaign.id,
                ...
                clicks: campaign.adClicks,
                spend: campaign.adSpend,
                ctr: campaign.impressions
                    ? (campaign.adClicks / campaign.impressions) * 100
                    : 0
            }));
        ...
        console.error('[Ad Studio v3 analytics]', error.message);
        res.status(500).json({ error: 'Analytics unavailable' });
    }
});

app.post('/api/ad-studio/v3/validate', async (req, res) => {
    try {
        const body = req.body || {};
        const errors = aspV3Errors(body);
        const reservation = await aspV3Reservation(body);
        const user = await aspV3User(String(body.userId || ''));
        const balance = aspV3Int(user?.points);
        ...
        res.json({
            ...
            reservation,
            balance,
            affordable: balance >= reservation.total,
            checkedAt: Date.now()
        });
    } catch (error) {
        console.error('[Ad Studio v3 validate]', error.message);
        res.status(500).json({ error: 'Validation unavailable' });
    }
});

app.post('/api/ad-studio/v3/campaigns', async (req, res) => {
    try {
        const body = req.body || {};
        ...
        const result = await aspV3Lock(`user:${userId}`, async () => {
            const user = await aspV3User(userId);
            ...
            if (balance < reservation.total) {
                throw new Error(
                    `Not enough Gems. Required ${reservation.total}, available ${balance}`
                );
            }

            const campaignId =
                `aspv3_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
            ...
            const campaign = {
                id: campaignId,
                userId,
                name: aspV3String(body.name, 80),
                link: aspV3String(body.link, 2048),
                desc: aspV3String(body.desc, 500),
                headline: aspV3String(body.headline, 100),
                cta: aspV3String(body.cta || 'Visit Now', 32),
                imageUrl: aspV3String(body.imageUrl, 2048),
                videoUrl: aspV3String(body.videoUrl, 2048),
                icon: aspV3String(body.icon || 'fa-rectangle-ad', 40),
                mode,
                isAdvertisement: mode !== 'task',
                sponsoredTask: mode !== 'advertisement',
                taskType: mode !== 'advertisement'
                    ? aspV3String(body.taskType || 'auto', 20)
                    : '',
                maxUsers: mode !== 'advertisement'
                    ? aspV3Int(body.maxUsers)
                    : 0,
                reward: mode !== 'advertisement'
                    ? aspV3Int(body.reward)
                    : 0,
                impressionBudget: mode !== 'task'
                    ? aspV3Int(body.impressionBudget)
                    : 0,
                dailyBudget: aspV3Int(body.dailyBudget),
                placements: Array.isArray(body.placements)
                    ? body.placements.slice(0, 10)
                    : ['home'],
                priority: aspV3Int(body.priority),
                impressions: 0,
                views: 0,
                adClicks: 0,
                adSpend: 0,
                ...
                paused: false,
                autoRenew: false,
                costPerImpression: reservation.costPerImpression,
                escrowReserved: reservation.total,
                adEscrowRemaining: reservation.advertisement,
                taskEscrowRemaining: reservation.task,
                createdAt: timestamp,
                updatedAt: timestamp,
                adStudioVersion: AD_STUDIO_PRO_V3.version
            };

            await dbSet(`campaigns/${campaignId}`, campaign);

            await dbUpdate(`users/${userId}`, {
                points: balance - reservation.total,
                stuckBalance:
                    aspV3Int(user.stuckBalance) + reservation.total,
                logs: aspV3Audit(
                    user,
                    `Created Ad Studio PRO v3 campaign ${campaignId}`,
                    reservation
                )
            });
        ...
        res.json({
            success: true,
            ...result,
            user: await aspV3User(userId),
            campaigns: await aspV3Owned(userId),
            generatedAt: Date.now()
        });
    } catch (error) {
        console.error('[Ad Studio v3 create]', error.message);
        res.status(400).json({ error: error.message || 'Campaign creation failed' });
    }
});

app.post('/api/ad-studio/v3/campaign/action', async (req, res) => {
    try {
        ...
        if (!userId || !campaignId) {
            return res.status(400).json({
                error: 'User and campaign are required'
            });
        }

        const result = await aspV3Lock(`campaign:${campaignId}`, async () => {
            const campaign = await aspV3Campaign(campaignId);

            if (!campaign || String(campaign.userId) !== userId) {
            ...
            const adRemaining = aspV3Int(campaign.adEscrowRemaining);
            const taskRemaining =
                Math.max(
                    0,
                    aspV3Int(campaign.maxUsers - campaign.claims) *
                        aspV3Int(campaign.reward)
                );

            const refund = adRemaining + taskRemaining;

            await dbUpdate(`users/${userId}`, {
                points: aspV3Int(user.points) + refund,
                stuckBalance: Math.max(
                    0,
                    aspV3Int(user.stuckBalance) - refund
                ),
                logs: aspV3Audit(
                    user,
                    `Liquidated Ad Studio PRO v3 campaign ${campaignId}`,
                    { refund }
                )
            });
        ...
        console.error('[Ad Studio v3 action]', error.message);
        res.status(400).json({ error: error.message || 'Campaign action failed' });
    }
});

app.get('/api/ad-studio/v3/inventory', async (req, res) => {
    try {
        const placement = aspV3String(req.query.placement || 'home', 40);
        const all = await dbGet('campaigns') || {};

        const inventory = Object.values(all)
            .filter(campaign => campaign.isAdvertisement)
            .filter(campaign => aspV3Status(campaign) === 'active')
            .filter(campaign => {
            ...
            .sort((a, b) => {
                ...
                return (
                    (a.impressions / Math.max(1, a.impressionBudget)) -
                    (b.impressions / Math.max(1, b.impressionBudget))
                );
            });
        ...
        console.error('[Ad Studio v3 inventory]', error.message);
        res.status(500).json({ error: 'Inventory unavailable' });
    }
});

app.post('/api/ad-studio/v3/impression', async (req, res) => {
    try {
        const campaignId = String(req.body?.campaignId || '');
        const userId = String(req.body?.userId || '');
        const placement = aspV3String(req.body?.placement || 'home', 40);

        const result = await aspV3Lock(
            `campaign:${campaignId}`,
            async () => {
                const campaign = await aspV3Campaign(campaignId);

                if (!campaign || !campaign.isAdvertisement) {
                    throw new Error('Advertisement unavailable');
                }

                if (campaign.paused) {
                    throw new Error('Advertisement is paused');
                }

                const impressions =
                    aspV3Int(campaign.impressions || campaign.views);
                const budget = aspV3Int(campaign.impressionBudget);
                const cost = Math.max(
                    1,
                    aspV3Int(campaign.costPerImpression, 1)
                );
                const escrow = aspV3Int(campaign.adEscrowRemaining);

                if (impressions >= budget) {
                    throw new Error('Advertisement budget exhausted');
                }

                if (escrow < cost) {
                    throw new Error('Advertisement escrow exhausted');
                }

                const nextImpressions = impressions + 1;
                const nextSpend = aspV3Int(campaign.adSpend) + cost;
                const nextEscrow = escrow - cost;

                await dbUpdate(`campaigns/${campaignId}`, {
                    impressions: nextImpressions,
                    views: nextImpressions,
                    adSpend: nextSpend,
                    adEscrowRemaining: nextEscrow,
                    escrowReserved: Math.max(
                        0,
                        aspV3Int(campaign.escrowReserved) - cost
                    ),
                    updatedAt: Date.now(),
                ...
                const sponsor = await aspV3User(campaign.userId);

                if (sponsor) {
                    await dbUpdate(`users/${campaign.userId}`, {
                        stuckBalance: Math.max(
                            0,
                            aspV3Int(sponsor.stuckBalance) - cost
                        )
                    });
                }
        ...
        res.status(409).json({
            error: error.message || 'Impression could not be recorded'
        });
    }
});

app.post('/api/ad-studio/v3/click', async (req, res) => {
    try {
        const campaignId = String(req.body?.campaignId || '');
        ...
        const result = await aspV3Lock(
            ...
            async () => {
                ...
                const clicks = aspV3Int(campaign.adClicks) + 1;
        ...
        res.status(404).json({
            error: error.message || 'Click could not be recorded'
        });
    }
});

app.get('/api/ad-studio/v3/report/:userId', async (req, res) => {
    try {
        const campaigns = await aspV3Owned(String(req.params.userId || ''));
        ...
        const rows = campaigns.map(campaign => ({
            ...
            ctr: campaign.impressions
                ...
                : 0,
            spend: campaign.adSpend,
            reserved: campaign.escrowReserved,
        ...
        console.error('[Ad Studio v3 report]', error.message);
