...
// Setup queue dict where key is submissionId (id)
for (let i = 0; i < maxUsers; i++) {
    queueDict['sub_' + i] = {
        id: 'sub_' + i,
        userId: 'user_' + i,
...
// 2b. Object keys iteration for checking if user is already in queue
console.time('Object find key by userId (User)');
for (let i = 0; i < ITERATIONS; i++) {
    let targetKey;
    for (const key in queueDict) {
        if (queueDict[key].userId === targetUser) {
            targetKey = key;
            break;
        }
    }
}
console.timeEnd('Object find key by userId (User)');
